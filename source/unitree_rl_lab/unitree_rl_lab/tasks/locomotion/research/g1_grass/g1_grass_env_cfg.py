...
DEFAULT_FIXED_STAGE_STEPS = [2_000_000] * (GRASS_TERRAIN_SCHEDULE.num_stages() - 1)
DEFAULT_SUCCESS_THRESHOLD = 0.85
DEFAULT_MIN_STEPS_PER_STAGE = 1_000_000
DEFAULT_CONTACT_THRESHOLD = 0.10
DEFAULT_POSTURE_THRESHOLD = 0.15
DEFAULT_COMPENSATION_THRESHOLD = 0.20
...
def _configure_grass_variant(
    ...
    *,
    ...
    _apply_friction_range_to_existing_randomization(cfg, active_stage)
...
def _apply_friction_range_to_existing_randomization(cfg: Any, stage: GrassTerrainStage) -> None:
    ...
    physics_material_event = getattr(cfg.events, "physics_material", None)
    if physics_material_event is not None:
        physics_material_event.params["static_friction_range"] = friction_range
        physics_material_event.params["dynamic_friction_range"] = friction_range
...
def _make_fixed_schedule_progression() -> FixedScheduleProgression:
    return FixedScheduleProgression(stage_steps=list(DEFAULT_FIXED_STAGE_STEPS))


def _make_success_gate_progression() -> SuccessRateGateProgression:
    return SuccessRateGateProgression(
        success_threshold=DEFAULT_SUCCESS_THRESHOLD,
        min_steps_per_stage=DEFAULT_MIN_STEPS_PER_STAGE,
    )
